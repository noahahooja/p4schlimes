...
playerlist1 = ["Kobe Bryant,", "Lebron James", "Lonzo Ball"]

playerlist2 = ["Kobe Bryant,", "Lebron James", "Lonzo Ball"]
...
class Players:
    ...
    def player_series(self):
        limit = self._series
        f = [(random.sample((playerlist1), k=2))]  # fibonacci starting array/list
        while limit > 0:
            self.set_data(f[0])
            f = [f[0]]
            limit -= 1
...
if __name__ == "__main__":
    ...
    playerrecs = Player(a/a)
    print(f"Here are some player recomendations = {playerrecs.list}")

#for i in range(a):
    #print(f"Listing of Player Recs {i}= {playerrecs.get_sequence(i)}")
